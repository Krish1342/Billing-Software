...
class InvoicePDFGenerator:
    ...
    def generate_invoice_pdf(
        ...
        info_table.setStyle(
            TableStyle(
                [
                    ...
                    ("FONTNAME", (2, 0), (2, -1), "Times-New-Roman"),
                    ("FONTSIZE", (0, 0), (-1, -1), 9),
        ...
        # Line items table with totals included at bottom
        rate_header_para = Paragraph(
            '<font name="Times-New-Roman">Rate</font> <font size="8" name="Helvetica">(with making charges)</font>',
            styles["Normal"],
        )
        table_data = [["#", "Description", "HSN", "Qty", rate_header_para, "Amount"]]
        ...
        # Append totals rows inside the same table
        summary_start_row = len(table_data)
        subtotal_str = self._format_decimal(invoice_data.get("subtotal", "0"), 2)
        cgst_str = self._format_decimal(invoice_data.get("cgst", "0"), 2)
        sgst_str = self._format_decimal(invoice_data.get("sgst", "0"), 2)
        final_total_str = self._format_decimal(invoice_data.get("final_total", "0"), 2)
        rounded_off_val = Decimal(str(invoice_data.get("rounded_off", "0")))

        table_data.append(["", "", "", "", "Subtotal:", subtotal_str])
        table_data.append(
            ["", "", "", "", f"CGST @ {self.settings['tax']['cgst_rate']}%:", cgst_str]
        )
        table_data.append(
            ["", "", "", "", f"SGST @ {self.settings['tax']['sgst_rate']}%:", sgst_str]
        )
        if rounded_off_val != Decimal("0"):
            table_data.append(
                [
                    "",
                    "",
                    ...
                    "",
                    "Rounded Off:",
                    self._format_decimal(str(rounded_off_val), 2),
                ]
            )
        # Final total row
        final_row_index = len(table_data)
        table_data.append(["", "", "", "", "Final Total:", final_total_str])
        final_row_index = (
            final_row_index  # index of the just-appended 'Final Total' row
        )

        # Create table
        items_table = Table(table_data, colWidths=[25, 200, 50, 50, 80, 65])
