...
class InvoiceUI:
    ...
    def setup_ui(self):
        ...
        ttk.Label(header_frame, text="Invoice No:").grid(
        ...
        ttk.Label(header_frame, text="Date:").grid(
        ...
        # Row 1: Customer Name
        ttk.Label(header_frame, text="Customer Name:").grid(
        ...
        ttk.Label(header_frame, text="Address:").grid(
        ...
        ttk.Label(header_frame, text="Phone:").grid(
        ...
        ttk.Label(header_frame, text="GSTIN:").grid(
        ...
        ttk.Label(input_frame, text="HSN:").grid(
            row=0, column=2, sticky=tk.W, padx=(0, 5)
        )
        ...
        ttk.Label(input_frame, text="Qty:").grid(
        ...
        ttk.Label(input_frame, text="Rate:").grid(
        ...
        scrollbar = ttk.Scrollbar(
            table_frame, orient=tk.VERTICAL, command=self.items_tree.yview
        )
        ...
        ttk.Button(
            item_controls, text="Remove Selected", command=self.remove_line_item
        ...
        ttk.Label(
            totals_frame, textvariable=self.subtotal_var, font=("Arial", 10)
        ).grid(row=0, column=1, sticky=tk.E, padx=(10, 0))

        ttk.Label(
            totals_frame, text=f"CGST @ {self.settings['tax']['cgst_rate']}%:"
        ...
        ttk.Label(
            totals_frame, text=f"SGST @ {self.settings['tax']['sgst_rate']}%:"
        ...
        ttk.Label(
            totals_frame, textvariable=self.total_gst_var, font=("Arial", 10)
        ).grid(row=3, column=1, sticky=tk.E, padx=(10, 0))
        ...
        ttk.Label(
            totals_frame, textvariable=self.final_total_var, font=("Arial", 12, "bold")
        ).grid(row=6, column=1, sticky=tk.E, padx=(10, 0))

        # Optional: User-specified total
        ttk.Label(totals_frame, text="Override Total:").grid(
            row=7, column=0, sticky=tk.W, pady=(10, 0)
        )
        self.override_total_var = tk.StringVar()
        override_entry = ttk.Entry(
            totals_frame, textvariable=self.override_total_var, width=15
        )
        ...
        ttk.Button(
            actions_frame, text="Save Invoice", command=self.save_invoice, width=15
        ).grid(row=0, column=0, padx=(0, 5))

        ttk.Button(
            actions_frame, text="Generate PDF", command=self.generate_pdf, width=15
        ).grid(row=0, column=1, padx=(0, 5))

        ttk.Button(
            actions_frame, text="New Invoice", command=self.new_invoice, width=15
        ).grid(row=0, column=2, padx=(0, 5))

        ttk.Button(
            actions_frame, text="Load Invoice", command=self.load_invoice, width=15
    ...
    def add_line_item(self):
        ...
        if provided < 2:
            messagebox.showwarning(
                "Validation", "Please provide at least 2 of: Quantity, Rate, Amount"
            )
            return
    ...
    def recalculate_totals(self):
        ...
        try:
            totals = self.calculator.calculate_invoice_totals(
                self.line_items, override_total
            )
    ...
    def save_invoice(self):
        ...
        try:
            ...
            messagebox.showinfo(
                "Success", f"Invoice saved successfully!\nInvoice ID: {invoice_id}"
            )
    ...
    def generate_pdf(self):
        ...
        try:
            ...
            self.pdf_generator.generate_invoice_pdf(
                filename, invoice_data, line_items_str
            )
    ...
    def load_invoice(self):
        ...
        ttk.Button(button_frame, text="Load", command=do_load).pack(
            side=tk.LEFT, padx=5
        )
