...
class SupabaseDatabaseManager:
    ...
    def update_category(self, category_id: str, name: str, description: str = None) -> bool:
        """Update a category."""
        try:
    ...
    def delete_category(self, category_id: str) -> bool:
        ...
        try:
            ...
            inventory_check = self.supabase.table('inventory').select('id').eq('category_id', category_id).limit(1).execute()
            if inventory_check.data:
                raise ValueError("Cannot delete category that is being used by inventory items")
    ...
    def get_suppliers(self) -> List[Dict]:
        """Get all suppliers."""
        try:
            result = self.supabase.table('suppliers').select('*').order('name').execute()
            return result.data or []
        except Exception as e:
    ...
    def add_supplier(self, name: str, code: str, contact_person: str = None, 
        ...
        """Add a new supplier."""
        try:
            result = self.supabase.table('suppliers').insert({
                'name': name,
                'code': code,
    ...
    def update_supplier(self, supplier_id: str, name: str, code: str, 
                       contact_person: str = None, phone: str = None, 
                       email: str = None, address: str = None) -> bool:
        """Update a supplier."""
        try:
            result = self.supabase.table('suppliers').update({
                'name': name,
                'code': code,
                'contact_person': contact_person,
                'phone': phone,
    ...
    def delete_supplier(self, supplier_id: str) -> bool:
        ...
        try:
            ...
            inventory_check = self.supabase.table('inventory').select('id').eq('supplier_id', supplier_id).limit(1).execute()
            if inventory_check.data:
                raise ValueError("Cannot delete supplier that is being used by inventory items")
            
            result = self.supabase.table('suppliers').delete().eq('id', supplier_id).execute()
            return len(result.data) > 0
        except ValueError:
            raise
        except Exception as e:
        ...
        try:
            ...
            for item in result.data or []:
                products.append({
                    ...
                    'category_item_id': item['category_item_no'],  # Add this for UI display
    ...
    def add_product(self, name: str, description: str = None, hsn_code: str = None,
        ...
        try:
            ...
            for i in range(quantity):
                result = self.supabase.rpc('add_inventory_item', {
    ...
    def update_product(self, product_id: str, name: str = None, description: str = None,
                      hsn_code: str = None, gross_weight: float = None, 
                      net_weight: float = None, quantity: int = None,
                      category_id: str = None, supplier_id: str = None,
                      melting_percentage: float = None, **kwargs) -> bool:
        ...
        try:
            ...
            result = self.supabase.table('inventory').update(update_data).eq('id', product_id).execute()
            return len(result.data) > 0
        except Exception as e:
    ...
    def delete_product(self, product_id: str) -> bool:
        ...
        try:
            ...
            item_check = self.supabase.table('inventory').select('status').eq('id', product_id).execute()
            ...
            result = self.supabase.table('inventory').delete().eq('id', product_id).execute()
            return len(result.data) > 0
        except ValueError:
            raise
        except Exception as e:
    ...
    def get_customers(self) -> List[Dict]:
        """Get all customers."""
        try:
            result = self.supabase.table('customers').select('*').order('name').execute()
            return result.data or []
        except Exception as e:
            print(f"Error getting customers: {e}")
            return []
    
    def add_customer(self, name: str, phone: str = None, email: str = None,
                    address: str = None, gstin: str = None) -> str:
        """Add a new customer."""
        try:
    ...
    def create_invoice(self, invoice_data: Dict) -> Dict:
        ...
        try:
            ...
            for item in invoice_data.get('items', []):
                items.append({
                    'inventory_id': item.get('product_id'),  # Map product_id to inventory_id
        ...
        try:
            ...
            for item in line_items:
                ...
                if not item_data['inventory_id']:
                    warnings.append(f"Item '{item_data['product_name']}' is not linked to inventory")
    ...
    def get_invoices(self, limit: int = 100) -> List[Dict]:
        """Get recent invoices."""
        try:
            result = self.supabase.table('bills').select('*').order('created_at', desc=True).limit(limit).execute()
            return result.data or []
        except Exception as e:
    ...
    def reverse_invoice(self, invoice_id: str) -> bool:
        """Reverse an invoice."""
        try:
            result = self.supabase.rpc('reverse_bill', {'p_bill_id': invoice_id}).execute()
            return result.data
        except Exception as e:
    ...
    # Stock movements
    def get_stock_movements(self, product_id: str = None, limit: int = 200) -> List[Dict]:
        """Get stock movements."""
        try:
    ...
    def get_sold_items(self, limit: int = 100) -> List[Dict]:
        ...
        try:
            result = self.supabase.table('sold_items_view').select('*').limit(limit).execute()
            return result.data or []
        except Exception as e:
    ...
    def reduce_stock(self, product_id: str, quantity: int = 1):
        ...
        try:
            ...
            result = self.supabase.table('inventory').update({
                'status': 'SOLD'
            }).eq('id', product_id).eq('status', 'AVAILABLE').execute()
    ...
    def get_sales_summary(self, from_date: str = None, to_date: str = None) -> Dict:
        ...
        try:
            ...
            if bills:
                ...
                items_result = self.supabase.table('bill_items').select('*').in_('bill_id', bill_ids).execute()
            ...
            for category in categories:
                ...
                if available_items <= threshold:
                    low_stock.append({
                        ...
                        'unit_price': 0.0,  # Default value since we don't store unit prices
                        'available_gross_weight': float(category.get('available_gross_weight', 0)),
                        'available_net_weight': float(category.get('available_net_weight', 0)),
